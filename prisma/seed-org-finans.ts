// prisma/seed-org-finans.ts
// F1 — Organizasyon Şeması: Finans-Muhasebe Müdürlüğü PİLOT verisi
//
// ÇALIŞTIRMA (HENÜZ ÇALIŞTIRILMADI — Elif onayı bekliyor):
//   npx tsx prisma/seed-org-finans.ts
//
// İdempotent: her çalıştırmada YALNIZ "ORG-FN" ve "ORG-FN-*" kod uzayı
// temizlenip yeniden kurulur. ORG-IV/ORG-FB/ORG-SA dahil başka hiçbir bölüme dokunulmaz.
//
...
// vekilPersonnelId null bırakılır (ileride canlı seçim kancası) — Satınalma/Fabrika
// seed'lerindeki güncel mantıkla aynı.
//
// Not (P06): Tabloda "Muhasebe Uzmanı (2)" olarak verilmiş — Fabrika P27'deki
// mantıkla aynı: "(2)" yalnız P04 ile kod ayrımı için tablo notasyonu, gerçek unvan
// P04 ile birebir aynı ("Muhasebe Uzmanı") kabul edildi, rol eşleştirmesi buna göre.
...
// ─── NORMALIZE (İK/Fabrika/Satınalma seed'indeki AYNI fonksiyon) ─────────
...
const NODES: SeedNode[] = [
  {
    ...
    parentCode: null,
    unitType: "DEPARTMENT",
    name: "Finans-Muhasebe Müdürlüğü",
    level: 0,
    sortOrder: 0,
  ...
  {
    code: "ORG-FN-P02",
    parentCode: "ORG-FN-P01",
    unitType: "POSITION",
    name: "Muhasebe Müdür Yardımcısı",
    level: 2,
    sortOrder: 0,
    approvedHeadcount: 1,
    employees: ["Mehtap Kaptan"],
  },
  {
    code: "ORG-FN-P03",
    parentCode: "ORG-FN-P02",
    unitType: "POSITION",
    name: "Muhasebe Finans Uzmanı",
    level: 3,
    sortOrder: 0,
    approvedHeadcount: 1,
    employees: [],
    vekaletDurumu: true,
    vekilAdi: "Mehtap Kaptan",
  },
  {
    code: "ORG-FN-P04",
    parentCode: "ORG-FN-P02",
    ...
    sortOrder: 1,
    approvedHeadcount: 1,
    employees: ["Pelin Banu Küçük"],
  },
  {
    code: "ORG-FN-P05",
    parentCode: "ORG-FN-P02",
    unitType: "POSITION",
    name: "Genel Muhasebe Sekreteri",
    level: 3,
    sortOrder: 2,
    approvedHeadcount: 1,
    employees: ["Ayser Davulcu"],
  },
  {
    code: "ORG-FN-P06",
    parentCode: "ORG-FN-P02",
    unitType: "POSITION",
    // Not: tabloda "(2)" ile verilmiş, P04 ile aynı gerçek unvan kabul edildi (bkz. dosya başı notu)
    name: "Muhasebe Uzmanı",
    level: 3,
    sortOrder: 3,
    approvedHeadcount: 1,
    employees: ["Öznur Erdoğan Topal"],
  },
];

// ─── SORUMLU TABLOSU (yalnız ORG-FN düğümü, EK-A A.17) ───────────────────

const SORUMLULUKLAR: { sira: number; birinciSorumlu: string; yedekSorumlu: string }[] = [
  { sira: 1, birinciSorumlu: "Ebru İleri", yedekSorumlu: "Mehtap Kaptan" },
  { sira: 2, birinciSorumlu: "Mehtap Kaptan", yedekSorumlu: "Pelin Banu Küçük" },
  { sira: 3, birinciSorumlu: "Pelin Banu Küçük", yedekSorumlu: "Öznur Erdoğan Topal" },
];
...
async function main() {
  console.log("F1 — Finans-Muhasebe Müdürlüğü pilotu yükleniyor...\n");

  // ── 1) İDEMPOTENT TEMİZLİK — yalnız ORG-FN / ORG-FN-* kapsamı ──────────
  // Tam eşleşme "ORG-FN" ya da "ORG-FN-" öneki: "ORG-FNX" gibi bir kod
  // yanlışlıkla yakalanmaz (startsWith("ORG-FN") tek başına yakalardı).
  const mevcut = await prisma.orgUnit.findMany({
    where: { OR: [{ code: "ORG-FN" }, { code: { startsWith: "ORG-FN-" } }] },
    select: { id: true },
  });
  ...
  if (mevcutIds.length > 0) {
    ...
    console.log(`Temizlendi: ${mevcutIds.length} eski ORG-FN* OrgUnit kaydı.\n`);
  }
  ...
  // ── 5) OrgBolumMeta + OrgSorumluluk (yalnız ORG-FN) ─────────────────────
  const orgFnId = idByCode.get("ORG-FN")!;
  ...
  // ── 6) RAPOR ─────────────────────────────────────────────────────────────
  console.log("=== F1 SEED RAPORU — Finans-Muhasebe Müdürlüğü Pilotu ===\n");
  ...
  console.log("\nSorumlu tablosu özeti (ORG-FN):");
  SORUMLULUKLAR.forEach((s) => {
    console.log(`  ${s.sira} | ${s.birinciSorumlu} | ${s.yedekSorumlu}`);
  });

  console.log("\nF1 Finans-Muhasebe Müdürlüğü pilotu tamamlandı.");
}

// prisma/seed-org-kurul-bgys.ts
// F1 — Organizasyon Şeması: BGYS (Bilgi Güvenliği Yönetim Sistemi) Kurulu
// PİLOT verisi (EK-A A.3)
//
// ÇALIŞTIRMA (HENÜZ ÇALIŞTIRILMADI — Elif onayı bekliyor):
//   npx tsx prisma/seed-org-kurul-bgys.ts
//
// İdempotent: her çalıştırmada YALNIZ "ORG-KR-BGYS" ve "ORG-KR-BGYS-*" kod uzayı
// temizlenip yeniden kurulur. Diğer kurullara (ORG-KR-IZIN, ORG-KR-ETIK,
// ORG-KR-DISIPLIN dahil) ve departmanlara (ORG-IV/FB/SA/FN/MH/KL/ST/AS/SS/SG)
// dokunulmaz.
//
...
//
// HİYERARŞİ (Disiplin/Etik Kurulu ile aynı desende — 2 SEVİYE): P01 (Ekip
// Lideri) kökün çocuğu, P02 (Yönetim Temsilcisi) P01'in çocuğu, P03/P04/P05/P06
// (Network Sorumlusu + Üyeler) P02'nin çocuğu.
//
// Not (P03 Network Sorumlusu, P06 3. Üye): EK-A'da isim yok → BOŞ kadro
// (vekalet DEĞİL).
// Not (P04/P05): aynı unvan "BGYS Üyesi", farklı kod — kaynak (EK-A A.3) böyle,
// rol (Position) eşleşmesi unvan bazlı olduğu için sorun yaratmaz.
// Not (sorumlu tablosu 3./4. satır): EK-A'da "Koray İleri" kısa formu — ağaçtaki
// tam isim "Koray Mert İleri" ile aynı kişi, aynen (kısaltılmış) korundu.
...
// ─── YAPI TANIMI (2 seviye — Network Sorumlusu + Üyeler Yönetim Temsilcisi'ne bağlı) ──
...
const NODES: SeedNode[] = [
  ...
  {
    code: "ORG-KR-BGYS-P01",
    parentCode: "ORG-KR-BGYS",
    unitType: "POSITION",
    name: "BGYS Ekip Lideri (GYS)",
    level: 1,
    sortOrder: 0,
    approvedHeadcount: 1,
  ...
  {
    code: "ORG-KR-BGYS-P02",
    parentCode: "ORG-KR-BGYS-P01",
    unitType: "POSITION",
    name: "BGYS Yönetim Temsilcisi",
    level: 2,
    sortOrder: 0,
    approvedHeadcount: 1,
  ...
  {
    code: "ORG-KR-BGYS-P03",
    parentCode: "ORG-KR-BGYS-P02",
    unitType: "POSITION",
    name: "BGYS Network Sorumlusu",
    level: 3,
    sortOrder: 0,
    approvedHeadcount: 1,
    employees: [], // BOŞ kadro
  },
  {
    ...
    sortOrder: 1,
    approvedHeadcount: 1,
    employees: ["Koray Mert İleri"],
  },
  {
    code: "ORG-KR-BGYS-P05",
    parentCode: "ORG-KR-BGYS-P02",
    unitType: "POSITION",
    name: "BGYS Üyesi (2)",
    level: 3,
...
// ─── SORUMLU TABLOSU (yalnız ORG-KR-BGYS düğümü, EK-A A.3) ───────────────

const SORUMLULUKLAR: { sira: number; birinciSorumlu: string; yedekSorumlu: string | null }[] = [
  { sira: 1, birinciSorumlu: "Gürhan Horbay", yedekSorumlu: "Fazıl Melih Dilben" },
  { sira: 2, birinciSorumlu: "Fazıl Melih Dilben", yedekSorumlu: "Elif Kasar" },
  { sira: 3, birinciSorumlu: "Koray İleri", yedekSorumlu: "Elif Kasar" },
  { sira: 4, birinciSorumlu: "Elif Kasar", yedekSorumlu: "Koray İleri" },
];
...
async function main() {
  console.log("F1 — BGYS Kurulu pilotu yükleniyor...\n");

  // ── 1) İDEMPOTENT TEMİZLİK — yalnız ORG-KR-BGYS / ORG-KR-BGYS-* kapsamı ─
  const mevcut = await prisma.orgUnit.findMany({
    where: { OR: [{ code: "ORG-KR-BGYS" }, { code: { startsWith: "ORG-KR-BGYS-" } }] },
    select: { id: true },
  });
  ...
  if (mevcutIds.length > 0) {
    ...
    console.log(`Temizlendi: ${mevcutIds.length} eski ORG-KR-BGYS* OrgUnit kaydı.\n`);
  }
  ...
  // ── 5) OrgBolumMeta + OrgSorumluluk (yalnız ORG-KR-BGYS) ────────────────
  const orgKrBgysId = idByCode.get("ORG-KR-BGYS")!;

  await prisma.orgBolumMeta.create({
    data: {
      orgUnitId: orgKrBgysId,
      dokumanNo: "IV-LS-45",
  ...
  // ── 6) RAPOR ─────────────────────────────────────────────────────────────
  console.log("=== F1 SEED RAPORU — BGYS Kurulu Pilotu ===\n");

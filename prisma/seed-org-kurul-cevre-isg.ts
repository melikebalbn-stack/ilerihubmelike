// prisma/seed-org-kurul-cevre-isg.ts
// F1 — Organizasyon Şeması: Çevre & İSG Yönetimi Kurulu PİLOT verisi (EK-A A.8)
//
// ÇALIŞTIRMA (HENÜZ ÇALIŞTIRILMADI — Elif onayı bekliyor):
//   npx tsx prisma/seed-org-kurul-cevre-isg.ts
//
// İdempotent: her çalıştırmada YALNIZ "ORG-KR-CEVREISG" ve "ORG-KR-CEVREISG-*"
// kod uzayı temizlenip yeniden kurulur. Diğer kurullara (ORG-KR-IZIN, ORG-KR-ETIK,
// ORG-KR-DISIPLIN, ORG-KR-BGYS, ORG-KR-KVKK dahil) ve departmanlara (ORG-IV/FB/
// SA/FN/MH/KL/ST/AS/SS/SG) dokunulmaz.
//
// Personnel ve Position tablolarına SADECE OKUMA (findMany) yapılır;
// create/update/delete kesinlikle yok.
//
// VEKALET YOK, BOŞ KADRO YOK — bu kurulda tüm kutular dolu.
//
// HİYERARŞİ (2 SEVİYE): P01 (Yönetim Temsilcisi) kökün çocuğu, P02 (Ekip Üyesi)
// P01'in çocuğu, P03/P04/P05/P06 (İSG Uzmanı, İşyeri Hekimi, Çevre Mühendisi,
// Atık Saha Sorumlusu) P02'nin çocuğu.
//
// Not (P06, Atık Saha Sorumlusu): ÇOKLU kutu — 2 isim (Alaaddin Erdoğan,
// Mustafa Cebeci) ayrı OrgEmployee kaydı, N=2.
// Not (P04, Dr. Mustafa Cengiz): normalize() "Dr." önekini temizler →
// "MUSTAFA CENGIZ" olarak Personnel'de aranır.
// Not (sorumlu tablosu "Elif Karadeniz"): ağaçta hiçbir kutuda geçmeyen ayrı bir
// isim — sorumlu tablosu serbest metin, OrgEmployee ile bağımlı değil, aynen
// kaynaktaki (EK-A A.8) gibi kullanıldı.
...
// ─── YAPI TANIMI (2 seviye — Ekip Üyesi'nin altında uzman/çoklu kutular) ─
...
const NODES: SeedNode[] = [
  {
    code: "ORG-KR-CEVREISG",
    parentCode: null,
    unitType: "DEPARTMENT",
    name: "Çevre & İSG Yönetimi",
    level: 0,
    sortOrder: 0,
  ...
  {
    code: "ORG-KR-CEVREISG-P01",
    parentCode: "ORG-KR-CEVREISG",
    unitType: "POSITION",
    name: "Çevre & İSG Yönetim Temsilcisi",
    level: 1,
    sortOrder: 0,
    approvedHeadcount: 1,
  ...
  {
    code: "ORG-KR-CEVREISG-P02",
    parentCode: "ORG-KR-CEVREISG-P01",
    unitType: "POSITION",
    name: "Çevre & İSG Ekip Üyesi",
    level: 2,
    sortOrder: 0,
    approvedHeadcount: 1,
  ...
  {
    code: "ORG-KR-CEVREISG-P03",
    parentCode: "ORG-KR-CEVREISG-P02",
    unitType: "POSITION",
    name: "İSG Uzmanı",
    level: 3,
    sortOrder: 0,
  ...
  {
    code: "ORG-KR-CEVREISG-P06",
    parentCode: "ORG-KR-CEVREISG-P02",
    unitType: "POSITION",
    name: "Atık Saha Sorumlusu",
    level: 3,
    sortOrder: 3,
    approvedHeadcount: 2,
    employees: ["Alaaddin Erdoğan", "Mustafa Cebeci"],
  },
];

// ─── SORUMLU TABLOSU (yalnız ORG-KR-CEVREISG düğümü, EK-A A.8) ───────────
...
const SORUMLULUKLAR: { sira: number; birinciSorumlu: string; yedekSorumlu: string | null }[] = [
  ...
  { sira: 2, birinciSorumlu: "Gökçe Ekşioğlu", yedekSorumlu: "Elif Karadeniz" },
  { sira: 3, birinciSorumlu: "Elif Karadeniz", yedekSorumlu: "Elif Kasar" },
];
...
async function main() {
  console.log("F1 — Çevre & İSG Yönetimi Kurulu pilotu yükleniyor...\n");

  // ── 1) İDEMPOTENT TEMİZLİK — yalnız ORG-KR-CEVREISG / ORG-KR-CEVREISG-* ─
  const mevcut = await prisma.orgUnit.findMany({
    where: { OR: [{ code: "ORG-KR-CEVREISG" }, { code: { startsWith: "ORG-KR-CEVREISG-" } }] },
    select: { id: true },
  });
  ...
  if (mevcutIds.length > 0) {
    ...
    console.log(`Temizlendi: ${mevcutIds.length} eski ORG-KR-CEVREISG* OrgUnit kaydı.\n`);
  }
  ...
  // ── 5) OrgBolumMeta + OrgSorumluluk (yalnız ORG-KR-CEVREISG) ────────────
  const orgKrCevreisgId = idByCode.get("ORG-KR-CEVREISG")!;

  await prisma.orgBolumMeta.create({
    data: {
      orgUnitId: orgKrCevreisgId,
      dokumanNo: "IV-LS-45",
  ...
  // ── 6) RAPOR ─────────────────────────────────────────────────────────────
  console.log("=== F1 SEED RAPORU — Çevre & İSG Yönetimi Kurulu Pilotu ===\n");
  ...
  console.log("\nSorumlu tablosu özeti (ORG-KR-CEVREISG):");
  SORUMLULUKLAR.forEach((s) => {
    console.log(`  ${s.sira} | ${s.birinciSorumlu} | ${s.yedekSorumlu ?? "(yedek yok)"}`);
  });

  console.log("\nF1 Çevre & İSG Yönetimi Kurulu pilotu tamamlandı.");
}

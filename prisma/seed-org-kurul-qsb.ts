// prisma/seed-org-kurul-qsb.ts
// F1 — Organizasyon Şeması: QSB Kurulu PİLOT verisi (EK-A A.11)
//
// ÇALIŞTIRMA (HENÜZ ÇALIŞTIRILMADI — Elif onayı bekliyor):
//   npx tsx prisma/seed-org-kurul-qsb.ts
//
// İdempotent: her çalıştırmada YALNIZ "ORG-KR-QSB" ve "ORG-KR-QSB-*" kod uzayı
// temizlenip yeniden kurulur. Diğer kurullara (ORG-KR-IZIN, ORG-KR-ETIK,
// ORG-KR-DISIPLIN, ORG-KR-BGYS, ORG-KR-KVKK, ORG-KR-CEVREISG, ORG-KR-MMOGLE
...
//
// UYARI — EK-A A.11 aslında bir MATRİS (sütunlu tablo, ağaç değil): tepede
// Gürhan Horbay, sütunlarda Lojistik/Satınalma/Kalite/Planlama/Üretim, hücreler
// kişi değil pozisyon türü. Bu seed, matrisi DÜZ 2 SEVİYE bir ağaca basitleştirir
// (P01 → P02-P07). Planlama/Üretim sütunları EK-A'da boştu, atlandı. Bu QSB'nin
// en belirsiz/en çok yorum gerektiren kurul olduğu kabul edilerek yapıldı —
// ekranda görülünce Elif değerlendirip gerekirse düzeltecek.
//
// Not (P02-P07): matris hücre başlıkları (pozisyon türü), EK-A'da isim yok →
// hepsi BOŞ kadro.
...
// ─── YAPI TANIMI (düz 2 seviye — matrisin basitleştirilmiş hali) ─────────
...
const NODES: SeedNode[] = [
  ...
  {
    code: "ORG-KR-QSB-P01",
    parentCode: "ORG-KR-QSB",
    unitType: "POSITION",
    name: "QSB Sorumlusu (Genel Müdür Yardımcısı)",
    level: 1,
    sortOrder: 0,
    approvedHeadcount: 1,
  ...
  {
    code: "ORG-KR-QSB-P02",
    parentCode: "ORG-KR-QSB-P01",
    unitType: "POSITION",
    name: "Müşteri Lojistiği Uzmanı (Yurt İçi)",
    level: 2,
    sortOrder: 0,
    approvedHeadcount: 1,
    employees: [], // BOŞ kadro
  },
  {
    ...
    parentCode: "ORG-KR-QSB-P01",
    unitType: "POSITION",
    name: "Müşteri Lojistiği Uzmanı (Yurt Dışı)",
    level: 2,
    sortOrder: 1,
    approvedHeadcount: 1,
    employees: [], // BOŞ kadro
  },
  {
    ...
    parentCode: "ORG-KR-QSB-P01",
    unitType: "POSITION",
    name: "Hammadde Satın Alma Uzmanı",
    level: 2,
    sortOrder: 2,
    approvedHeadcount: 1,
    employees: [], // BOŞ kadro
  },
  {
    ...
    parentCode: "ORG-KR-QSB-P01",
    unitType: "POSITION",
    name: "Proje/Hizmet Satın Alma Uzmanı",
    level: 2,
    sortOrder: 3,
    approvedHeadcount: 1,
    employees: [], // BOŞ kadro
  },
  {
    ...
    parentCode: "ORG-KR-QSB-P01",
    unitType: "POSITION",
    name: "Tedarikçi Kalite Mühendisi (SQA)",
    level: 2,
    sortOrder: 4,
    approvedHeadcount: 1,
    employees: [], // BOŞ kadro
  },
  {
    ...
    parentCode: "ORG-KR-QSB-P01",
    unitType: "POSITION",
    name: "Giriş Kalite Operatörleri",
    level: 2,
    sortOrder: 5,
    approvedHeadcount: 1,
    employees: [], // BOŞ kadro
...
// ─── SORUMLU TABLOSU (yalnız ORG-KR-QSB düğümü, EK-A A.11) ───────────────

const SORUMLULUKLAR: { sira: number; birinciSorumlu: string; yedekSorumlu: string | null }[] = [
  { sira: 1, birinciSorumlu: "Gürhan Horbay", yedekSorumlu: "Sami Tekoğlu" },
];
...
async function main() {
  console.log("F1 — QSB Kurulu pilotu yükleniyor...\n");

  // ── 1) İDEMPOTENT TEMİZLİK — yalnız ORG-KR-QSB / ORG-KR-QSB-* kapsamı ───
  const mevcut = await prisma.orgUnit.findMany({
    where: { OR: [{ code: "ORG-KR-QSB" }, { code: { startsWith: "ORG-KR-QSB-" } }] },
    select: { id: true },
  });
  ...
  if (mevcutIds.length > 0) {
    ...
    console.log(`Temizlendi: ${mevcutIds.length} eski ORG-KR-QSB* OrgUnit kaydı.\n`);
  }
  ...
  // ── 5) OrgBolumMeta + OrgSorumluluk (yalnız ORG-KR-QSB) ─────────────────
  const orgKrQsbId = idByCode.get("ORG-KR-QSB")!;

  await prisma.orgBolumMeta.create({
    data: {
      orgUnitId: orgKrQsbId,
      dokumanNo: "IV-LS-45",
  ...
  // ── 6) RAPOR ─────────────────────────────────────────────────────────────
  console.log("=== F1 SEED RAPORU — QSB Kurulu Pilotu ===\n");
  ...
  for (const r of boxRaporu) {
    console.log(
      `${r.kod.padEnd(14)} | ${r.unvan.padEnd(41)} | ${String(r.n).padEnd(1)} | ${String(r.m).padEnd(1)} | ${r.rozet.padEnd(12)} | ${r.vekalet}`
    );
  }
  ...
  console.log("\nSorumlu tablosu özeti (ORG-KR-QSB):");
  SORUMLULUKLAR.forEach((s) => {
    console.log(`  ${s.sira} | ${s.birinciSorumlu} | ${s.yedekSorumlu ?? "(yedek yok)"}`);
  });

  console.log("\nF1 QSB Kurulu pilotu tamamlandı — MATRİS→AĞAÇ basitleştirmesi ekranda gözden geçirilmeli.");
}

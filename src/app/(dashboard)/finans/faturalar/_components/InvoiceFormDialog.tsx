...
export function InvoiceFormDialog({ open, onOpenChange, onCreated }: Props) {
  ...
  useEffect(() => {
    ...
    fetch('/api/finans/faturalar/departments')
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        const res = await fetch(
          `/api/finans/faturalar/tcmb-rate?date=${form.invoiceDate}&currency=EUR`
        )
        if (!res.ok) return
        ...
        if (form.currency === 'TRY') {
          ...
          const usdRes = await fetch(
            `/api/finans/faturalar/tcmb-rate?date=${form.invoiceDate}&currency=USD`
          )
          if (!usdRes.ok) return

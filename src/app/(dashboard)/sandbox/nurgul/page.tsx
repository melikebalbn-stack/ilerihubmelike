...
export default async function NurgulSandboxPage() {
  return (
    <SandboxGuard slug="nurgul">
      <NurgulSandboxContent />
    </SandboxGuard>
  )
}

async function NurgulSandboxContent() {
  const session = await getServerSession(authOptions)
  const sandboxModule = getSandboxBySlug('nurgul')
  ...
  return (
    <div className="space-y-6">
      <SandboxHeader
        ...
        userName={session?.user?.name ?? 'Nurgül'}
      />
      <SandboxWorkspace ownerName="Nurgül" />
    </div>
  )
}

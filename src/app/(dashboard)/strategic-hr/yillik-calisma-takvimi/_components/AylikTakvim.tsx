...
export function AylikTakvim({ rows, yil, onRowClick }: { rows: YillikTakvimKaydiRow[]; yil: number; onRowClick: (id: string) => void }) {
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {AY_ISIMLERI.map((ay, index) => <section key={ay} className="min-h-36 rounded-lg border p-3">
        ...
        <div className="space-y-2">
          ...
          {aylaraGore[index].map(row => <button key={row.id} type="button" className="flex w-full gap-2 rounded p-1 text-left text-sm hover:bg-accent" onClick={() => onRowClick(row.id)}>

...
const THRESHOLD_MS = 5 * 60 * 1000; // 5 dakika
...
export async function POST(req: NextRequest) {
  ...
  for (const form of forms) {
    ...
    // Bekleme-başı = kararlı adımların MAX(decidedAt); yoksa pending.createdAt (submit anı)
    const decidedTimes = form.approvals
      .filter((a) => a.decision !== null && a.decidedAt)
      .map((a) => a.decidedAt!.getTime());
    const waitingSince = decidedTimes.length
      ? new Date(Math.max(...decidedTimes))
      : pending.createdAt;

    if (now.getTime() - waitingSince.getTime() <= THRESHOLD_MS) {
  ...
  return NextResponse.json({ ok: true, scanned: forms.length, sent, skipped });
}

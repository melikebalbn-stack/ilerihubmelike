...
const mocks = vi.hoisted(() => ({
  ...
  listServisGuzergahAracVarsayilanlari: vi.fn(),
  createServisGuzergahAracVarsayilan: vi.fn(),
}))

vi.mock('@/lib/auth/require-permission', () => ({ requirePermission: mocks.requirePermission }))
vi.mock('@/lib/servis-yonetimi/service', () => ({
  listServisGuzergahAracVarsayilanlari: mocks.listServisGuzergahAracVarsayilanlari,
  createServisGuzergahAracVarsayilan: mocks.createServisGuzergahAracVarsayilan,
}))
...
describe('GET /api/servis-yonetimi/guzergah/[id]/arac-varsayilan — permission guard', () => {
  it('servis.view izni olmayan kullanıcı 403 alır', async () => {
    ...
    expect(mocks.listServisGuzergahAracVarsayilanlari).not.toHaveBeenCalled()
  })

  it('servis.view izni olan kullanıcı listeyi görür', async () => {
    mocks.requirePermission.mockResolvedValue(permissionResult(true))
    mocks.listServisGuzergahAracVarsayilanlari.mockResolvedValue([{ id: 'v1' }])
    const res = await GET(new NextRequest('http://localhost/x'), context)
    const json = await res.json()
    expect(res.status).toBe(200)
    expect(json.toplam).toBe(1)
    expect(mocks.listServisGuzergahAracVarsayilanlari).toHaveBeenCalledWith('guzergah-1', { aktif: undefined })
  })
})

describe('POST /api/servis-yonetimi/guzergah/[id]/arac-varsayilan — permission guard', () => {
  const body = { dilimId: 'dilim-1', aracId: 'arac-1', rol: 'ANA', baslangicTarihi: '2026-01-01' }
  ...
  it('servis.tanim.manage izni olmayan kullanıcı 403 alır', async () => {
    ...
    expect(res.status).toBe(403)
    expect(mocks.createServisGuzergahAracVarsayilan).not.toHaveBeenCalled()
  })
  ...
  it('servis.tanim.manage izni olan kullanıcı atama oluşturabilir, createdById session kullanıcısıdır', async () => {
    ...
    mocks.createServisGuzergahAracVarsayilan.mockResolvedValue({ id: 'v1', ...body })
    const res = await POST(new NextRequest('http://localhost/x', { method: 'POST', body: JSON.stringify(body) }), context)
    const json = await res.json()
    expect(res.status).toBe(201)
    expect(json.ok).toBe(true)
    expect(mocks.createServisGuzergahAracVarsayilan).toHaveBeenCalledWith(

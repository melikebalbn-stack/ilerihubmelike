      ...
      {canManage && (
        <div className="shrink-0 inline-flex items-center gap-1">
          ...
          <Button
            ...
            disabled={baslik}
            onClick={onSil}
            title={baslik ? 'Alt kodu olan kayıt silinemez' : 'Sil'}
            className={!baslik ? 'text-red-600 hover:text-red-700 hover:bg-red-50' : ''}
          >
...
export function HataKoduAgacClient({ canManage }: { canManage: boolean }) {
  ...
  async function sil() {
    ...
    try {
      const res = await fetch(`/api/quality/hata-kodu/${hedef.id}`, { method: 'DELETE' })
      const json = await res.json().catch(() => null)
      // 409 mesajı API'den geldiği gibi gösterilir ("... aktif=false yapın").
      if (!res.ok) throw new Error(json?.error || 'Silme başarısız')
      toast.success(`${hedef.kod} silindi`)
      setSilinecek(null)
      fetchList()
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      ...
      {canManage && (
        <AlertDialog open={silinecek !== null} onOpenChange={(v) => !v && setSilinecek(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Hata kodu silinsin mi?</AlertDialogTitle>
              <AlertDialogDescription>
                <span className="font-quality-mono">{silinecek?.kod}</span> — {silinecek?.ad}
                <br />
                Yalnız hiç kullanılmamış kodlar silinebilir. Kullanımdaysa sunucu reddeder ve
                kodu pasif yapmanız istenir.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>

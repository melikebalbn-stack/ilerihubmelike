...
export async function generateVisitReportPDF(report: VisitReport) {
  console.log("generateVisitReportPDF başladı")
  ...
  try {
    ...
    // Load Poppins fonts and logo
    const { PoppinsRegular, PoppinsBold, PoppinsSemiBold } = await import("./fonts/poppins")
    const { IleriGroupLogo } = await import("./fonts/logo")
    ...
    // Helper: Content with left accent border (handles page breaks properly)
    function addContentWithAccent(text: string, accentColorRGB: [number, number, number]) {
      ...
      lines.forEach((line: string, index: number) => {
        if (checkPageBreak(6)) {
          // New page started, add continuation indicator
        }

        // Draw accent bar for each line
    ...
    // Logo (1050x308 original = 3.41:1 ratio)
    try {
      const logoWidth = 35
    ...
    // Creation date
    doc.setFontSize(6)
    doc.text(`${format(new Date(report.createdAt), "dd.MM.yyyy HH:mm")}`, pageWidth - margin, 24, { align: "right" })
    ...
    let infoRows = 3 // Saat, Firma, Ziyaret Türü
    ...
    const visitDateStr = format(new Date(report.visitDate), "d MMMM yyyy, EEEE", { locale: tr })
    addInfoRow("Ziyaret Tarihi", visitDateStr)

    if (report.endDate) {
      const endDateStr = format(new Date(report.endDate), "d MMMM yyyy", { locale: tr })
    ...
    if (report.additionalNotes) {
      ...
      addContentWithAccent(report.additionalNotes, [230, 126, 34]) // Orange accent
      yPos += 6
    }
    ...
    if (report.nextSteps) {
      ...
      addContentWithAccent(report.nextSteps, [39, 174, 96]) // Green accent
      yPos += 6
    }
...
export async function downloadVisitReportPDF(report: VisitReport) {
  try {
...
export async function getVisitReportPDFBlob(report: VisitReport): Promise<Blob> {
  const doc = await generateVisitReportPDF(report)
  return doc.output("blob")
}

export async function getVisitReportPDFBase64(report: VisitReport): Promise<string> {
  const doc = await generateVisitReportPDF(report)
